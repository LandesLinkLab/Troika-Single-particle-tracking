function [map_new, termied] = linking_map_R(map_pr, map1_2, position, time, termi, search_r)
...
i = i + 1; % continuing beyond the length of the prior map_new
for j = max(map_pr(:,2)) + 1 : numel(map1_2(:,2)) 
    if ~any(map_new(:,2)== map1_2(j, 2)) %added this if statement
