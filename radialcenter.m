...
%m_cop = 1;toc
%tic;
%m_cop = -(fdv+fdu)*j + (fdv-fdu);%figure;imagesc((angle(m_cop)));
%figure;imagesc(abs(m_cop))
...
b = ym - m.*xm;
%b(find(abs(b(:))>10^5)) = b(find(abs(b(:))>10^5)+1);
...
%xcentroid = 24 - (Nx+1)/2.0;
%ycentroid = 17 - (Ny+1)/2.0;
%{
test = zeros(Ny-1,Nx-1);
for xcentroid = 1 : Nx-1
    for ycentroid = 1 : Ny-1
        b = (ym-ycentroid) - m.*(xm-xcentroid);
w  = dImag2./sqrt((xm-xcentroid).*(xm-xcentroid)+(ym-ycentroid).*(ym-ycentroid));  
test(ycentroid,xcentroid) = sum(sum((b).^2./(m.^2+1).*w));
    end
end
figure
imagesc(test)
...
for xcentroid = 1 : Nx
    for ycentroid = 1 : Ny
w  = dImag2./sqrt((xm-xcentroid).*(xm-xcentroid)+(ym-ycentroid).*(ym-ycentroid));  
test(ycentroid,xcentroid) = sum(sum((b-ycentroid+m*xcentroid).^2./(m.^2+1).*w));
    end
end
figure
imagesc(test)
